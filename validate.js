class User {
  ...
  class Validate {
    ...
    require() {
      for(const [key, value] of this.formdata.entries()) {
        if(!Boolean(value)) {
        let error2 = `${key} không được bỏ trống`;
        this.errors.push([key, [error2]])
        }
      }
      return this.errors;
    }
  ...
  submitBtn.addEventListener('click', function () {
      ...
      let errors = new Validate(formdata).require();
      reseterror();
      if (errors.length > 0) {
        for (let [key, loi] of errors) {
        document.querySelector(`.er-${key}`).innerHTML = loi
        }
        return;
      }
